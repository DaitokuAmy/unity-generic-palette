...
`Unity Generic Palette` は、`Color` や `TextStyle` のような共通設定を `EntryId` でひも付け、複数の UI や表示要素から再利用できるようにするライブラリです。
...
`PaletteAsset` で共通化したい設定項目を定義し、各 `Applier` に `EntryId` を指定してその値を参照します。結果として、同じ `EntryId` を参照している複数の UI や表示要素に対して、色や文字スタイルのような共通設定を一か所の定義でそろえられます。

## Features

- `Applier` に `EntryId` を指定するだけで、色や文字スタイルのような設定を共通化できる
- 同じ `EntryId` を参照する複数の UI や表示要素を、一か所の定義でまとめて管理できる
- `Profile` を切り替えることで、共通化した設定をテーマや言語ごとにまとめて差し替えられる
- `ProfileAsset` を独立したアセットとして扱えるため、Addressables での分割管理や遅延ロードと相性がよい
- 組み込みの `Color` / `TMP TextStyle` / `Legacy TextStyle` / `Gradient` パレット型をすぐ使い始められる
- 独自の `PaletteAsset` / `Applier` / `Loader` を追加しやすく、プロジェクトに合わせて拡張しやすい
...
4. 各 `Applier` が対応する `EntryId` の値を受け取り、対象へ反映する
...
```text
https://github.com/DaitokuAmy/unity-generic-palette.git?path=/Packages/com.daitokuamy.unitygenericpalette#0.9.1
```
...
`Included ProfileAssets` は、あらかじめ使う `ProfileAsset` を `PaletteEngine` に直接持たせておくための設定です。動的な Load / Unload が不要な場合は、生成した `ProfileAsset` をここへまとめて登録しておくのが一番シンプルです。

この運用では、初期化後の `ChangeProfileAsync(...)` でも登録済み `ProfileAsset` がそのまま使われるため、別途 `Loader` を用意する必要はありません。

`Included ProfileAssets` に入れた Profile は `Loader` より優先されます。つまり、一部の Profile だけは常駐させたい、残りだけを動的ロードしたい、という混在運用もできます。

### 4. Initialize at runtime

最小構成では、`Start` などから `PaletteEngine.InitializeAsync()` を呼びます。動的ロードを使う構成では、`Loader` を引数に渡して初期化します。
...
`Loader` を使う設計なのに、引数なしの `InitializeAsync()` を先に呼ぶと、後続の Profile 切り替えで必要なロード処理を行えません。なお、`Included ProfileAssets` だけで完結する構成なら、引数なしの `InitializeAsync()` でも動作します。
...
- `PaletteEngine.InitializeAsync(IPaletteProfileLoader loader)`
...
`PaletteProfileAsset` を Addressable にしたうえで、`PaletteAssetBase` に自動同期される `ProfileId -> GUID` 対応表を使って読み込みます。動的ロードを前提にする場合は、`Loader` を渡した `InitializeAsync(...)` のあとで `ChangeProfileAsync(...)` を呼んでください。
...
### 6. Create a custom `Loader`

`ProfileAsset` を Addressables 以外の取得元から読みたい場合は、`IPaletteProfileLoader` を実装します。たとえば `Resources/PaletteProfiles/{profileId}` という配置ルールでロードするなら、次のような最小実装にできます。

`ResourcesPaletteProfileLoader.cs`
...
namespace App.Palette {
    public sealed class ResourcesPaletteProfileLoader : IPaletteProfileLoader {
        ...
        public void Unload(string profileId, string profileGuid, PaletteProfileAssetBase profileAsset) {
            ...
            Resources.UnloadAsset(profileAsset);
        }
    }
}
```
...
namespace App.Palette {
    public sealed class PaletteBootstrap : MonoBehaviour {
        ...
        private async UniTask InitializeAsync() {
            await PaletteEngine.InitializeAsync(new ResourcesPaletteProfileLoader());
        }
    }
}
```

`PaletteEngine.ChangeProfileAsync<TProfileAsset>(profileId)` が呼ばれると、未ロードの Profile はこの `Loader` 経由で取得され、差し替え前に `Loader` 管理下の旧 Profile があれば `Unload` も呼ばれます。
...
サンプルでは `PaletteEngine.InitializeAsync(new GuidBaseAddressablesLoader())` で初期化し、Profile を切り替えたときに複数の UI 要素へ変更が連動する様子を確認できます。
